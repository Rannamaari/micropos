<x-filament-panels::page>
    ...
    <div class="space-y-6">
        <section class="rounded-xl border border-gray-200 bg-white p-5 shadow-sm dark:border-white/10 dark:bg-white/5">
            ...
            @if ($branch)<p class="mt-4 text-sm text-gray-600 dark:text-gray-300">Sales values are in <strong>{{ $branch->currency }}</strong>. Tender collections are shown separately in each currency, without recalculating historic payments at today’s exchange rate.</p>@endif
        </section>
        ...
        @if ($branch)
            ...
            <section class="grid gap-4 lg:grid-cols-2">
                <div class="rounded-xl border border-gray-200 bg-white p-5 dark:border-white/10 dark:bg-white/5">
                    ...
                    <dl class="mt-3 grid gap-2 sm:grid-cols-2">@foreach ($report['paymentCollections'] as $currency => $amount)<div class="rounded-lg bg-gray-50 px-3 py-2 dark:bg-white/5"><dt class="text-xs font-medium uppercase tracking-wide text-gray-500">Collected · {{ $currency }}</dt><dd class="mt-1 text-lg font-semibold">{{ $currency }} {{ number_format($amount, 2) }}</dd></div>@endforeach</dl>
                    <dl class="mt-3 space-y-2 text-sm">@forelse ($report['payments'] as $payment)<div class="flex justify-between gap-3"><dt>{{ ucfirst(str_replace('_', ' ', $payment->payment_method)) }} · {{ $payment->currency }}</dt><dd class="text-right">{{ $payment->currency }} {{ number_format((float) $payment->currency_total, 2) }}@if ($payment->currency !== $branch->currency)<br><span class="text-xs text-gray-500">{{ $branch->currency }} {{ number_format((float) $payment->total, 2) }} applied</span>@endif</dd></div>@empty <p class="text-gray-500">No payments recorded for this period.</p>@endforelse</dl>
                </div>
                <div class="rounded-xl border border-gray-200 bg-white p-5 dark:border-white/10 dark:bg-white/5">
            ...
            <section class="rounded-xl border border-gray-200 bg-white p-5 dark:border-white/10 dark:bg-white/5">
                ...
                <div class="mt-4 overflow-x-auto"><table class="w-full text-sm"><thead class="text-left text-gray-500"><tr><th class="pb-3">Date</th><th class="pb-3 text-right">Transactions</th><th class="pb-3 text-right">Gross Sales</th><th class="pb-3 text-right">Returns</th><th class="pb-3 text-right">Net Sales</th><th class="pb-3 text-right">Collections Received</th></tr></thead><tbody>@forelse ($report['dailySales'] as $day)<tr class="border-t border-gray-100 dark:border-white/10"><td class="py-3 font-medium"><button type="button" wire:click="selectDailySalesDate('{{ $day['date'] }}')" class="text-primary-600 hover:underline">{{ \Illuminate\Support\Carbon::parse($day['date'])->format('M d, Y') }}</button></td><td class="py-3 text-right">{{ $day['transactions'] }}</td><td class="py-3 text-right">{{ $branch->currency }} {{ number_format($day['sales_total'], 2) }}</td><td class="py-3 text-right text-danger-600">{{ $branch->currency }} {{ number_format($day['returns_total'], 2) }}</td><td class="py-3 text-right font-semibold">{{ $branch->currency }} {{ number_format($day['net_sales'], 2) }}</td><td class="py-3 text-right">@foreach ($day['payment_collections'] as $currency => $amount)<div>{{ $currency }} {{ number_format($amount, 2) }}</div>@endforeach</td></tr>@empty <tr><td colspan="6" class="py-4 text-gray-500">No sales recorded for this period.</td></tr>@endforelse</tbody></table></div>
            </section>
            ...
            @if ($dailySalesDate)
                ...
                <section class="rounded-xl border border-gray-200 bg-white p-5 dark:border-white/10 dark:bg-white/5">
                    <div><h2 class="text-lg font-semibold">Daily Transaction Details</h2><p class="text-sm text-gray-500">Completed sales for {{ \Illuminate\Support\Carbon::parse($dailySalesDate)->format('M d, Y') }}.</p></div>
                <div class="mt-4 overflow-x-auto"><table class="w-full text-sm"><thead class="text-left text-gray-500"><tr><th class="pb-3">Bill No.</th><th class="pb-3">Time</th><th class="pb-3">Customer</th><th class="pb-3">Cashier</th><th class="pb-3">Payment Method</th><th class="pb-3 text-right">Bill Amount</th><th class="pb-3 text-right">Tax</th><th class="pb-3 text-right">Total</th><th class="pb-3 text-right">Collections Received</th><th class="pb-3 text-right">Balance</th></tr></thead><tbody>@forelse ($report['dailyTransactions'] as $sale)<tr class="border-t border-gray-100 dark:border-white/10"><td class="py-3 font-medium">{{ $sale['sale_number'] }}</td><td class="py-3">{{ $sale['completed_at'] ?? '—' }}</td><td class="py-3">{{ $sale['customer'] }}</td><td class="py-3">{{ $sale['cashier'] }}</td><td class="py-3">{{ $sale['payment_methods'] }}</td><td class="py-3 text-right">{{ $branch->currency }} {{ number_format($sale['subtotal'], 2) }}</td><td class="py-3 text-right">{{ $branch->currency }} {{ number_format($sale['tax'], 2) }}</td><td class="py-3 text-right font-semibold">{{ $branch->currency }} {{ number_format($sale['grand_total'], 2) }}</td><td class="py-3 text-right">{{ $sale['payment_collections'] }}</td><td class="py-3 text-right {{ $sale['balance_due'] > 0 ? 'text-warning-600 font-semibold' : '' }}">{{ $branch->currency }} {{ number_format($sale['balance_due'], 2) }}</td></tr>@empty <tr><td colspan="10" class="py-4 text-gray-500">No completed sales recorded for this date.</td></tr>@endforelse</tbody></table></div>
                </section>
            @endif
